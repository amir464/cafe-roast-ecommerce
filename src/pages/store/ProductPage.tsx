  ...
  return (
    <div className="mx-auto max-w-7xl px-3 pb-[90px] pt-[30px] sm:px-4 md:px-7">
      ...
      <section className="grid grid-cols-1 gap-8 lg:grid-cols-2 lg:gap-[60px]">
        <div>
          <img
            className="aspect-square h-auto w-full rounded-[22px] object-cover lg:h-[520px] lg:aspect-auto"
            src={mainImage}
            alt={product.title}
          />
          ...
          <div className="mt-2.5 flex gap-2.5 overflow-x-auto pb-1.5">
            {product.gallery.map((image, index) => (
              <button
                ...
                className={`h-[82px] w-[82px] shrink-0 overflow-hidden rounded-[10px] border-2 ${
                  mainImage === image
                    ? "border-[var(--primary)]"
                    : "border-transparent"
                }`}
        ...
        <div>
          <span className="inline-flex items-center rounded-[30px] bg-[color-mix(in_srgb,var(--success)_24%,var(--surface))] px-[13px] py-2 text-[13px] font-semibold text-[color-mix(in_srgb,var(--success)_55%,var(--text-primary))]">
          ...
          <div className="my-[25px] flex flex-wrap items-center gap-3">
            ...
            {product.oldPrice > 0 && (
              <>
                ...
                <b className="rounded-lg bg-[var(--accent)] px-[9px] py-[5px] text-[var(--on-accent)]">
          ...
          <ul className="grid list-none grid-cols-1 p-0 md:grid-cols-2">
            {product.features.map((feature) => (
              <li
                className="m-[7px] flex items-start gap-[7px] text-[var(--text-secondary)]"
                key={feature}
              >
          ...
          <div className="mt-[25px] grid grid-cols-[1fr_48px] gap-2.5 sm:flex">
            ...
            <button
              className="col-start-1 inline-flex min-h-[46px] items-center justify-center gap-2 rounded-xl bg-[var(--primary)] px-[22px] py-3 font-semibold text-[var(--on-dark-primary)] hover:bg-[var(--primary-hover)] sm:flex-1"
            ...
            <button
              className={`col-start-2 inline-flex min-h-[46px] items-center justify-center rounded-xl border border-[var(--border)] bg-[var(--surface)] p-3 ${
                isFavorite
                  ? "text-[var(--favorite)] [&_svg]:fill-current"
                  : "text-[var(--favorite-inactive)] [&_svg]:fill-transparent"
              }`}
          ...
          <div className="mt-[25px] grid grid-cols-1 gap-3 border-t border-[var(--border)] pt-5 sm:flex sm:flex-wrap sm:justify-between [&_span]:flex [&_span]:items-center [&_span]:gap-[5px] [&_svg]:w-[17px]">
            <span>
              <Truck />
              ارسال سریع
            </span>
